...
- [x] `functions/jest.config.js` — NEW (ts-jest preset)
- [x] `functions/package.json` — add `@google-cloud/vertexai: ^1.12.0`, devDeps `jest`, `@types/jest`, `ts-jest`, `@firebase/rules-unit-testing: ^5.0.1`; script `"test": "jest"`
- [x] `functions/src/lib/quota.ts` — NEW (`QUOTA_TZ`, `getDhakaDateKey`, `monthKey`)
- [x] `functions/src/lib/gemini.ts` — FILL (replaces Phase 2 stub)
- [x] `functions/src/lib/rate_limit.ts` — FILL (replaces Phase 2 stub)
- [x] `functions/src/index.ts` — ADD `mentorBotChat` export alongside existing `ping`
- [x] `functions/src/__tests__/quota.test.ts`, `rate_limit.test.ts`, `gemini.test.ts`, `idempotency.test.ts`, `usage_log.test.ts` — NEW unit tests
- [x] `functions/tool/verify-model-availability.js` — NEW one-shot Vertex model-resolve smoke (executor runs before pinning the model ID in gemini.ts)

**PR-2 (rules + tests + docs):**
- [x] `firestore.rules` — three path lock insertions per D-17
- [x] `functions/src/__tests__/rules.test.ts` — NEW rules tests for AI-08
- [x] `BACKEND_SETUP.md` — append `## Phase 3 — Vertex AI + Key Rotation` section (gcloud IAM + key rotation + budget raise + MONTHLY_CALL_CEILING)

**PR-3 (client swap + cleanup):**
- [x] `pubspec.yaml` — add `uuid: ^4.5.3`; remove `google_generative_ai: ^0.4.6`
- [x] `lib/core/constants/quota.dart` — NEW
- [x] `lib/data/repositories/mentor_bot_repository.dart` — NEW
- [x] `lib/data/models/mentor_bot_response.dart` — NEW
- [x] `lib/features/tutor/chat_viewmodel.dart` (or wherever the viewmodel lives post-Phase-1 D-02 refactor; verify path) — SWAP to repository
- [x] `lib/core/services/gemini_service.dart` — DELETE
- [x] `.vscode/launch.json`, `.github/workflows/ci.yml`, README, BACKEND_SETUP.md — remove `--dart-define=GEMINI_API_KEY` everywhere
- [x] `integration_test/mentor_bot_smoke_test.dart` — NEW
- [x] `.github/workflows/ci.yml` `functions:` job — ADD `npm test` step
...
- [x] Feedback latency < 140 s for full suite
- [x] `uuid ^4.5.3` + (gemini-3.1-pro OR gemini-2.5-pro fallback) resolved cleanly under existing version constraints
- [x] `functions/package-lock.json` updated to include `@google-cloud/vertexai` + Jest deps; `npm ci` exits 0 in CI
- [x] Manual gates documented in BACKEND_SETUP.md §Phase 3 (model verification + key rotation + Vertex IAM + budget raise)
- [x] `nyquist_compliant: true` set in this frontmatter once every row turns ✅ (or is explicitly ⏸ with a Phase 5+ follow-up entry in STATE.md)

**Approval:** closed by Plan 03-15 on 2026-05-20
