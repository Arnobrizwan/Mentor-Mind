...
- **D-02:** **`AppleProvider.appAttestWithDeviceCheckFallback`** on iOS 14+ release builds. **Debug provider** on dev simulators + CI. Provider selection lives in `main.dart` immediately after `Firebase.initializeApp(...)` and BEFORE any provider/repository reads — wired via `FirebaseAppCheck.instance.activate(...)`. Release vs dev branch via `kReleaseMode` (Flutter constant).
  - **AMENDED 2026-05-19:** Originally locked as `AppleProvider.appAttest` with `Runner.entitlements` `com.apple.developer.devicecheck.appattest.environment = production`. Amended to `appAttestWithDeviceCheckFallback` because the Apple Developer Program account is on the FREE tier (paid enrollment not currently justified by solo dev). App Attest capability + entitlements require paid enrollment; DeviceCheck does not. The fallback provider silently uses DeviceCheck when App Attest is unavailable, preserving `enforceAppCheck: true` semantics without the paid-tier dependency. The `Runner.entitlements` change from the original D-02 is DROPPED. When/if paid enrollment lands (Phase 6+), revisit to upgrade to pure `AppleProvider.appAttest`.
